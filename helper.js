...
var waitElementToDisappear = function(locator) {
	browser.wait(function() {
		return browser.isElementPresent(locator) // if element is already
													// present, this line will
													// evaluate to true
		.then(function(presenceOfElement) {
			return !presenceOfElement
		}); // this modifies the previous line so that it evaluates to false
			// until the element is no longer present.
	}, 10000);
}
...
/**
 * Usage: selectDropdownByNumber ( element, index) element : select element
...
/**
 * Usage: selectDropdownByText (selector, item) selector : select element item :
 * option(s) in the dropdown.
 */
...
var setBrowserParams = function() {
	browser.driver.manage().window().setSize(browser.params.screenWidth,
			browser.params.screenHeight);
